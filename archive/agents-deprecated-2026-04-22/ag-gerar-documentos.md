...
### Coordinator (ag-gerar-documentos)
...
12. **Fluxo obrigatorio** — Ler → Mapear layouts → Text transforms (idx antigos) → Notes → Reorder → Design (idx novos) → Font pass → Salvar → **Spell Check (ag-revisar-ortografia)** → Validar.
...
- Usar `~/.claude/lib/xlsx_components.py` para funcoes reutilizaveis
...
8. Acentuacao PT-BR OBRIGATORIA em todo texto (verificado por ag-revisar-ortografia spell check)

### FASE 3: Validacao

1. **Spell Check (ag-revisar-ortografia)**: `python3 ~/.claude/scripts/spellcheck_document.py <file>` — corrige ortografia e acentuacao silenciosamente
...
- Zero ghost text?
- Zero sobreposicao?
- Ortografia e acentuacao corretas? (ag-revisar-ortografia spell check)
