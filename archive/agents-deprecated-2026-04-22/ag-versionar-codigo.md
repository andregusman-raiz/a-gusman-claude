...
```
/ag-versionar-codigo branch [nome]    -> Cria feature branch a partir de main
/ag-versionar-codigo commit           -> Commit com mensagem semantica
/ag-versionar-codigo pr               -> Cria PR com template completo
/ag-versionar-codigo tag [versao]     -> Cria tag de release
/ag-versionar-codigo changelog        -> Atualiza changelog
/ag-versionar-codigo release [versao] -> Changelog + tag + GitHub Release
/ag-versionar-codigo cleanup          -> Remove branches mergeadas locais
```
...
Executar ANTES de qualquer modo. Parar imediatamente se qualquer check falhar.

### 1. Path Absoluto
Se o path do repo não foi fornecido no prompt → PARAR com:
"ERRO: Path absoluto do repo é obrigatório. Exemplo: ~/Claude/GitHub/example-platform/"

### 2. Branch Atual
```bash
git -C [path] branch --show-current
```
Se retornar `main`, `master` ou `develop` → PARAR com:
"ERRO: Você está em branch protegida '[branch]'. Crie uma feature branch primeiro."

### 3. Credenciais (apenas modos pr e release)
```bash
bash ~/Claude/.claude/scripts/credential-preflight.sh [path]
```
Se exit code 2 → PARAR. Credenciais inválidas = operação vai falhar.

### 4. Git State Limpo
```bash
git -C [path] status --porcelain
```
Verificar se há arquivos em conflito (`UU`, `AA`, `DD`). Se houver → PARAR com:
"ERRO: Repo tem conflitos não resolvidos. Resolva antes de continuar."

### 5. Lint-Staged Orphans (não-bloqueante)
```bash
git -C [path] stash list | grep -c "lint-staged" || true
```
Se > 0 → WARN: "WARNING: Há stashes de lint-staged órfãos. Considere limpá-los com: git stash drop"
...
- **NUNCA mergear branches com testes falhando** — testes falhando = risco de regressao. Faca ag-testar-codigo passar primeiro.
- **NUNCA push direto em main** — sempre via feature branch + PR.

## Interacao com outros agentes

- ag-implementar-codigo (construir): chama ag-versionar-codigo ao final para commitar e criar PR
- ag-corrigir-bugs (bugfix): Gate 5 usa logica de commit do ag-versionar-codigo
- ag-pipeline-deploy (deploy-pipeline): deploy via PR merge, nao push direto
- ag-documentar-projeto (documentar): changelog gerado pelo ag-versionar-codigo
