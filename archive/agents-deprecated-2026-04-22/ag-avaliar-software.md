...
docker run --rm -v "$(pwd)/external-sw:/app" -w /app node:20-slim sh -c "bun install && bun run test"
...
docker run --rm -v "$(pwd)/external-sw:/app" -w /app node:20-slim sh -c "bun run build"
```

**REGRA DE SEGURANCA**: NUNCA rodar `bun install` de software desconhecido diretamente no host. Sempre usar Docker.
...
4. Explorar codebase com ag-explorar-codigo (subagent)
5. Analisar debito tecnico com ag-analisar-contexto (subagent)
6. Auditar seguranca com ag-verificar-seguranca (subagent)
...
- ag-explorar-codigo (explorar): mapear o codebase externo
- ag-analisar-contexto (analisar): diagnosticar debito tecnico
- ag-verificar-seguranca (auditar): auditoria de seguranca
- ag-planejar-incorporacao (planejar-incorporacao): recebe o relatorio como input
