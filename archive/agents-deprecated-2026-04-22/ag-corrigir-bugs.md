...
├── 1 bug obscuro                    → Chamar ag-depurar-erro (depurar) primeiro → depois FIX
...
## MODE: TRIAGE (era ag-corrigir-bugs)
...
- Se a causa nao for obvia → chamar ag-depurar-erro (depurar) primeiro
...
## MODE: BATCH (era ag-corrigir-bugs)
...
a. **Implementar** cada fix (invocar ag-depurar-erro se causa nao for obvia)
...
## MODE: PARALLEL (era ag-corrigir-bugs)
...
Usar `SendMessage` para comunicar progresso ao coordinator (ag-0-orquestrador) ou usuario:
...
- ag-depurar-erro (depurar): chamar quando causa raiz nao for obvia (modes FIX, BATCH, PARALLEL)
- ag-analisar-contexto (analisar): chamar se precisa entender contexto mais amplo (mode TRIAGE)
- ag-testar-codigo (testar): chamar apos cada sprint para validar (modes BATCH, PARALLEL)
- ag-validar-execucao (validar): chamar no final para verificar completude (modes BATCH, PARALLEL)
- ag-versionar-codigo (versionar): delegado para commits complexos e PRs (todos os modos)
- ag-0-orquestrador (orquestrar): reporta plano/resultado para decisao do usuario
