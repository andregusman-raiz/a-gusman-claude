...
type Category = "Data Display" | "Forms" | "Workflow" | "Layout" | "AI" | "Media" | "Tools" | "Export" | "Legal" | "Finance";
...
interface Solution {
  ...
  tags: string[];
  miniPreview: "kpi" | "table" | "form" | "timeline" | "shell" | "flow" | "chat" | "book" | "qr" | "code" | "kanban" | "docs" | "rag" | "calendar" | "bi" | "transcript" | "clm" | "studio" | "legal" | "datacatalog" | "appbuilder" | "tvcounter" | "radar" | "contractor" | "dre" | "audit-evidence" | "ad-analytics";
}
...
const solutions: Solution[] = [
  ...
  // ─── Padrões de novos repos (2026-03-27) ───
  { id: "dre-financial-dashboard", name: "Financial Drill-Down + Variance", desc: "DRE/P&L hierárquico com expand por nível, variância orçado×realizado, export multi-formato", icon: DollarSign, category: "Finance", complexity: "Muito Alta", tags: ["dre", "financeiro", "drill-down", "variância", "hierarquia", "export"], miniPreview: "dre" },
  { id: "audit-evidence-platform", name: "Evidence Audit + AI Analysis", desc: "Upload de evidências, análise AI de qualidade, approval chain, PDF async, compliance score", icon: ShieldCheck, category: "Workflow", complexity: "Alta", tags: ["auditoria", "evidência", "ai", "compliance", "aprovação", "score"], miniPreview: "audit-evidence" },
  { id: "ad-performance-analytics", name: "Ad Campaign Analytics + Alerts", desc: "Dashboard multi-plataforma (Meta/Google/TikTok), benchmark bands, budget alerts, role-aware views", icon: TrendingUp, category: "Data Display", complexity: "Alta", tags: ["ads", "analytics", "roas", "benchmark", "alertas", "multi-plataforma"], miniPreview: "ad-analytics" },
];

const ALL_CATEGORIES: Category[] = ["Data Display", "Forms", "Workflow", "Layout", "AI", "Media", "Tools", "Export", "Legal", "Finance"];
...
function MiniPreview({ type }: { type: Solution["miniPreview"] }) {
  ...
  return (
    <svg viewBox="0 0 120 64" className="h-full w-full" fill="none">
      ...
      {/* DRE — hierarchical drill-down table */}
      {type === "dre" && <>
        <rect x="2" y="4" width="116" height="56" rx="2" fill="currentColor" fillOpacity="0.03" stroke="currentColor" strokeOpacity="0.1" strokeWidth="0.5" />
        <rect x="6" y="8" width="108" height="6" rx="1" fill="currentColor" fillOpacity="0.06" />
        <rect x="10" y="10" width="24" height="2" rx="0.5" fill="#2D9E6B" fillOpacity="0.4" />
        <rect x="70" y="10" width="12" height="2" rx="0.5" fill="currentColor" fillOpacity="0.15" />
        <rect x="90" y="10" width="12" height="2" rx="0.5" fill="currentColor" fillOpacity="0.15" />
        {[18, 26, 34, 42].map((y, i) => <g key={`dre-${i}`}>
          <rect x={8 + i * 4} y={y} width={24 - i * 2} height="2" rx="0.5" fill="currentColor" fillOpacity={0.2 - i * 0.03} />
          <rect x="70" y={y} width="12" height="2" rx="0.5" fill="currentColor" fillOpacity="0.1" />
          <rect x="90" y={y} width="12" height="2" rx="0.5" fill={i === 2 ? "#DC3545" : "#2D9E6B"} fillOpacity="0.3" />
          {i < 2 && <polygon points={`${8 + i * 4 - 3},${y} ${8 + i * 4 - 3},${y + 2} ${8 + i * 4 - 1},${y + 1}`} fill={c} fillOpacity="0.3" />}
        </g>)}
        <line x1="6" y1="50" x2="114" y2="50" stroke="currentColor" strokeOpacity="0.1" strokeWidth="0.5" />
        <rect x="10" y="52" width="20" height="2.5" rx="0.5" fill={c} fillOpacity="0.4" />
        <rect x="90" y="52" width="14" height="2.5" rx="0.5" fill="#2D9E6B" fillOpacity="0.4" />
      </>}
      {/* Audit Evidence — upload + AI + approval chain */}
      {type === "audit-evidence" && <>
        <rect x="2" y="4" width="40" height="56" rx="2" fill="currentColor" fillOpacity="0.03" stroke="currentColor" strokeOpacity="0.1" strokeWidth="0.5" />
        <rect x="6" y="8" width="32" height="20" rx="2" fill="currentColor" fillOpacity="0.06" stroke={c} strokeOpacity="0.2" strokeWidth="0.5" strokeDasharray="2 1" />
        <text x="22" y="20" textAnchor="middle" fontSize="6" fill={c} fillOpacity="0.3">+</text>
        {[32, 40, 48].map((y, i) => <g key={`ev-${i}`}><rect x="6" y={y} width="32" height="5" rx="1" fill={i === 0 ? "#2D9E6B" : "currentColor"} fillOpacity={i === 0 ? 0.1 : 0.04} stroke="currentColor" strokeOpacity="0.08" strokeWidth="0.3" /><rect x="8" y={y + 1.5} width={16 + i * 3} height="2" rx="0.5" fill="currentColor" fillOpacity="0.12" /></g>)}
        <rect x="46" y="4" width="36" height="56" rx="2" fill="currentColor" fillOpacity="0.03" stroke="currentColor" strokeOpacity="0.1" strokeWidth="0.5" />
        <rect x="50" y="8" width="16" height="2.5" rx="0.5" fill="#A855F7" fillOpacity="0.3" />
        <text x="68" y="10" fontSize="4" fill="#A855F7" fillOpacity="0.4">AI</text>
        {[16, 26, 36].map((y) => <rect key={`ai-${y}`} x="50" y={y} width={24 + (y % 5) * 2} height="2" rx="0.5" fill="currentColor" fillOpacity="0.08" />)}
        <circle cx="64" cy="50" r="6" fill="#2D9E6B" fillOpacity="0.1" stroke="#2D9E6B" strokeOpacity="0.3" strokeWidth="0.5" />
        <text x="64" y="52" textAnchor="middle" fontSize="5" fill="#2D9E6B" fillOpacity="0.5">85</text>
        <rect x="86" y="4" width="32" height="56" rx="2" fill="currentColor" fillOpacity="0.03" stroke="currentColor" strokeOpacity="0.1" strokeWidth="0.5" />
        <line x1="94" y1="12" x2="94" y2="52" stroke="currentColor" strokeOpacity="0.1" strokeWidth="0.5" />
        {[12, 24, 36, 48].map((y, i) => <circle key={`ap-${i}`} cx="94" cy={y} r="2.5" fill={i < 2 ? "#2D9E6B" : i === 2 ? c : "currentColor"} fillOpacity={i < 3 ? 0.4 : 0.1} />)}
      </>}
      {/* Ad Analytics — metric cards with benchmark bands */}
      {type === "ad-analytics" && <>
        {[2, 42, 82].map((x, i) => <g key={`ad-${i}`}>
          <rect x={x} y="4" width="36" height="30" rx="2" fill="currentColor" fillOpacity="0.03" stroke="currentColor" strokeOpacity="0.1" strokeWidth="0.5" />
          <rect x={x + 4} y="8" width="14" height="2" rx="0.5" fill="currentColor" fillOpacity="0.15" />
          <rect x={x + 4} y="14" width="20" height="3" rx="0.5" fill={[c, "#5BB5A2", "#3B82F6"][i]} fillOpacity="0.5" />
          <rect x={x + 4} y="22" width="28" height="4" rx="1" fill="currentColor" fillOpacity="0.04" />
          <rect x={x + 4 + [8, 12, 6][i]} y="22" width="4" height="4" rx="0.5" fill={[c, "#5BB5A2", "#3B82F6"][i]} fillOpacity="0.3" />
          <line x1={x + 4 + 10} y1="22" x2={x + 4 + 10} y2="26" stroke="currentColor" strokeOpacity="0.15" strokeWidth="0.5" strokeDasharray="1 1" />
          <line x1={x + 4 + 20} y1="22" x2={x + 4 + 20} y2="26" stroke="currentColor" strokeOpacity="0.15" strokeWidth="0.5" strokeDasharray="1 1" />
        </g>)}
        <rect x="2" y="38" width="116" height="22" rx="2" fill="currentColor" fillOpacity="0.03" stroke="currentColor" strokeOpacity="0.1" strokeWidth="0.5" />
        {[10, 30, 50, 70, 90].map((x, i) => <rect key={`ab-${i}`} x={x} y={56 - [12, 18, 8, 14, 10][i]} width="14" height={[12, 18, 8, 14, 10][i]} rx="1" fill={c} fillOpacity={0.15 + i * 0.06} />)}
      </>}
    </svg>
  );
}
      ...
      <header className="border-b border-border">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-6">
          ...
          <div className="flex items-center gap-3">
            <Link href="/presentation" className="rounded-lg border border-border px-3 py-2 text-xs font-medium text-muted-foreground hover:border-[var(--raiz-orange)]/50 hover:text-[var(--raiz-orange)]">Presentation</Link>
            <Link href="/elements" className="rounded-lg border border-border px-3 py-2 text-xs font-medium text-muted-foreground hover:text-foreground">Elements</Link>
