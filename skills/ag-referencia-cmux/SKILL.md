...
- **Dock (0.64.22): existe, vem DESLIGADO** — ligar com `cmux right-sidebar set dock` (confirmar: `cmux right-sidebar mode` → `{"mode":"dock","visible":true}`; `cmux tree` mostra panes `[dock:global]`). Enquanto desligado, `new-pane --placement dock` responde `Dock placement is disabled` e `right-sidebar set dock` responde `not available` — **`disabled` é estado, não ausência de feature**: em 28/08 dois agentes leram isso como "não existe nesta versão" e o `dock.json` correto ficou inerte por horas. Controls em `~/.config/cmux/dock.json` (hoje: `feed tui`, `lazygit`, `de-prs-tui.sh`).
