...
2. **Aggregate** — rodar: `python -m scripts.aggregate_benchmark <workspace>/iteration-N --skill-name <name>` (executar do diretorio `~/.claude/skills/ag-criar-skill/`)
3. **Analyst pass** — ler benchmark e surfar padroes (ver `agents/analyzer.md`)
4. **Viewer** — gerar HTML:
   ```bash
   nohup python ~/.claude/skills/ag-criar-skill/eval-viewer/generate_review.py \
...
3. Rodar loop de otimizacao:
   ...
   python -m scripts.run_loop \
     ...
     --verbose
   ```
   (executar do diretorio `~/.claude/skills/ag-criar-skill/`)
