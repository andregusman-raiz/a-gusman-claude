...
# Ensure PATH includes common locations (needed for launchd context)
...
if ! command -v claude &>/dev/null; then
  echo "ERROR: claude CLI not found in PATH. Exiting." | tee -a "$LOG_FILE" 2>/dev/null
  exit 1
fi

# Verify python3 is available
if ! command -v python3 &>/dev/null; then
  echo "ERROR: python3 not found in PATH. Exiting." | tee -a "$LOG_FILE" 2>/dev/null
  exit 1
fi
...
echo "Projects: $PROJECT_COUNT | Skills: ${#SKILLS[@]} | Dry run: ${DRY_RUN:-no}" | tee -a "$LOG_FILE"
echo "" | tee -a "$LOG_FILE"
