---
name: ag-B-23-bugfix
description: "Bugfix unificado com auto-routing. Triage (classificar bugs), Fix (1 bug + 5 gates), Batch (2-5 bugs em sprints), Parallel (6+ bugs em Agent Teams). Substitui ag-B-23/24/25/26."
model: sonnet
...
| 1 bug obscuro | ag-B-09 first | Then `--fix` |
...
### Mode --triage
Apos classificar bugs, spawnar ag-M-50 para cada bug que NAO sera fixado nesta sessao:
```
Agent({
...
- ag-B-26 (fix-verificar) → mode `--fix`
- ag-B-23-bugfix-batch → mode `--batch`
