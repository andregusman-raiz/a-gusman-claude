...
# e2e-batch — E2E Tests with Infrastructure Validation
...
- Rodar suite E2E completa de forma confiavel (nao apenas `npx playwright test`)
...
if npx playwright test --project=smoke --reporter=list 2>/dev/null; then
  echo "Smoke tests passaram. Infraestrutura OK."
elif npx playwright test tests/e2e/smoke/ --reporter=list 2>/dev/null; then
  echo "Smoke tests passaram. Infraestrutura OK."
else
  ...
  if [ -n "$FIRST_SPEC" ]; then
    if npx playwright test "$FIRST_SPEC" --reporter=list; then
...
- Browser nao instala? `npx playwright install --with-deps`
...
```bash
npx playwright test "${BATCH[@]}" --reporter=json,list --output=/tmp/e2e-results/
```
...
npx playwright test "${FAILED_SPECS[@]}" --reporter=json,list
```
...
- Se falhas sao de codigo (nao de teste): reportar como bugs para ag-B-09 ou ag-B-23
