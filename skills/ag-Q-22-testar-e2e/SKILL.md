...
Diferenca de ag-Q-13: "a funcao retorna o valor certo" (logica) vs "o usuario consegue fazer login" (experiencia).
...
/e2e spec              -> Le spec (ag-P-06) e gera testes por criterio
...
**Com spec (ag-P-06):** extraia fluxos em tabela:
...
```bash
bunx playwright test --reporter=html,list          # todos os testes
bunx playwright test tests/e2e/auth/               # fluxo especifico
bunx playwright test --project=smoke                # so smoke tests
bunx playwright test --debug                       # browser visivel + inspector
bunx playwright show-report tests/e2e/report       # abrir HTML report
```
...
```bash
which playwright-cli || bun install -g @playwright/cli@latest
```
...
- `/ag-Q-36 [url]` — Teste exploratorio completo com relatorio (ag-Q-36-testar-manual-mcp)
- `/ag-Q-37 [fluxo]` — Gera testes Playwright a partir de fluxo CLI (ag-Q-37-gerar-testes-mcp)
- `/ag-D-38 [url]` — Smoke tests contra URL Vercel (ag-D-38-smoke-vercel)
...
```bash
# Preview deploy
PLAYWRIGHT_TEST_BASE_URL=https://app-preview-abc.vercel.app bunx playwright test --project=smoke

# Production
PLAYWRIGHT_TEST_BASE_URL=https://app.vercel.app bunx playwright test --project=smoke
```
...
jobs:
  qa:
    ...
    steps:
      ...
      - uses: actions/setup-node@v4
        with: { node-version: 20 }
      - run: bun install && bun install -g @playwright/cli@latest && bun run dev &
      - name: Claude QA
...
- **Sugestao:** ag-B-09 /depurar — [onde e o que investigar]
...
## Proximos Passos
1. ag-B-09 /depurar — [issue critico]
2. ag-B-08 /construir — [fix necessario]
```
...
**Piramide:** E2E (ag-Q-22) = poucos, lentos, alta confianca — so fluxos criticos. Integration = medios. Unit (ag-Q-13) = muitos, rapidos, baratos.

**Camadas de teste por ambiente:**
| Ambiente | Tipo | Trigger |
|----------|------|---------|
| Local | E2E completo + MCP exploratorio | `bun run dev` |
...
- Logica de negocio pura -> unit test (ag-Q-13)
