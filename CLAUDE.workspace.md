...
Audit raiz-platform (2026-04-25): `docs/diagnosticos/playwright-audit-2026-04-25.md`.
...
# Profile isolado (apps sem SSO: totvs-educacional, chamada-app, github)
bash ~/Claude/.claude/scripts/playwright-persistent-login.sh totvs-educacional
```
...
│   ├── raiz-platform/        # Cada projeto tem CLAUDE.md + skills proprias
...
Repos como `raiz-platform` tem `lint-staged` que roda `eslint --fix` + `prettier --write` durante
...
6. Regra completa: ver `.claude/rules/multi-agent-isolation.md` (R1-R5). Incidente de referencia: `docs/diagnosticos/2026-04-07-raiz-data-engine-wip-triage.md`
...
|---------|---------------|-------------|
| Matrículas, educacional, financeiro, metas | **PBI_RAIZ** (RAIZDB01) | TOTVS RM (fallback) |
...
Ver regra completa: `.claude/rules/sql-multi-db-governance.md` | Guide PBI_RAIZ: `unified/guides/pbi-raiz-bridge.md`
