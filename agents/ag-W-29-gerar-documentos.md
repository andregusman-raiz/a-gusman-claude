...
12. **Fluxo obrigatorio** — Ler → Mapear layouts → Text transforms (idx antigos) → Notes → Reorder → Design (idx novos) → Font pass → Salvar → **Spell Check (ag-W-31)** → Validar.
...
8. Acentuacao PT-BR OBRIGATORIA em todo texto (verificado por ag-W-31 spell check)

### FASE 3: Validacao

1. **Spell Check (ag-W-31)**: `python3 ~/.claude/scripts/spellcheck_document.py <file>` — corrige ortografia e acentuacao silenciosamente
...
- Zero ghost text?
- Zero sobreposicao?
- Ortografia e acentuacao corretas? (ag-W-31 spell check)
