...
```
/ag-criar-agente [descricao do agente desejado]     → Criar agente com descricao
/ag-criar-agente                                     → Modo interativo (entrevista)
```
...
- ag-melhorar-agentes e ag-criar-skill sao excecoes (nao seguem numeracao)
...
|-------------|--------|----------|
| Scans, lookups, verificacoes rapidas | haiku | ag-explorar-codigo, ag-validar-execucao, ag-saude-sessao |
| Implementacao, testes, reviews | sonnet | ag-implementar-codigo, ag-testar-codigo, ag-revisar-codigo |
| Arquitetura, specs, analise profunda, debug | opus | ag-analisar-contexto, ag-especificar-solucao, ag-depurar-erro |
...
Criar em `~/.claude/skills/agNN/SKILL.md` seguindo anatomia padrao (ver Fase 3 do ag-criar-skill).

### Fase 6: Registrar no Catalogo

Adicionar o novo agente ao catalogo do ag-0-orquestrador em `~/.claude/skills/ag-0-orquestrador/SKILL.md`:
...
# Description do agent aparece no catalogo do ag-0-orquestrador
grep "ag-NN" ~/.claude/skills/ag-0-orquestrador/SKILL.md
```
...
- **ag-0-orquestrador** (orquestrar): ag-criar-agente registra novos agentes no catalogo do ag-0-orquestrador
- **ag-melhorar-agentes** (melhorar): ag-melhorar-agentes pode solicitar criacao de agentes novos via ag-criar-agente
- **ag-criar-skill**: ag-criar-agente cria agentes, ag-criar-skill cria/melhora skills. Se o agente precisa de skill complexa, ag-criar-agente cria o esqueleto e ag-criar-skill refina
- **ag-revisar-codigo** (criticar): pode revisar o agent file como code review

## Anti-Patterns

- **NUNCA criar agente sem command file** — agente sem comando e inacessivel
- **NUNCA duplicar funcionalidade** — verificar catalogo antes de criar (grep no ag-0-orquestrador SKILL.md)
...
- **NUNCA pular registro no ag-0-orquestrador** — agente nao registrado e agente invisivel
...
- [ ] Registrado no catalogo do ag-0-orquestrador?
...
- Model: haiku|sonnet|opus
- Registrado no ag-0-orquestrador: sim
