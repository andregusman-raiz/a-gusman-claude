...
- Cada teammate com worktree isolation (herdado do ag-B-08)
...
Isso permite que o ag-M-00 (coordinator) acompanhe progresso em tempo real.
...
6. Se self-check passa → handoff para ag-Q-12

## Notificacoes via SendMessage

Quando em modo paralelo (Team) ou coordenado por ag-M-00:
...
- Ao finalizar self-check: `SendMessage("Self-check PASS. Pronto para handoff ag-Q-12.")`
...
Depois: tentar abordagem diferente ou escalar para ag-B-09.

## O que NÃO fazer

- Pesquisar antes de tentar (pesquisa é do ag-P-05)
- Refatorar enquanto constrói (refatoração é do ag-B-10)
- Otimizar antes de funcionar (otimização é do ag-B-11)
- Declarar "pronto" sem self-check
- Deixar TODOs/stubs e marcar como feito

## Interacao com outros agentes

- ag-B-09 (depurar): chamar quando erro nao e obvio — causa raiz, nao sintoma
- ag-Q-12 (validar): apos self-check, ag-Q-12 faz validacao independente
- ag-Q-13 (testar): apos implementacao, rodar testes (Green phase do TDD)
- ag-B-26 (fix-verificar): se typecheck/lint falha, pipeline com 5 gates
- ag-D-18 (versionar): para commits semanticos apos completar fase
- ag-Q-41 (QAT): apos construir feature que gera output (chat, imagem, relatorio), sugerir criacao de cenario QAT via `/ag-Q-41`
...
- **NUNCA ignorar erros de typecheck para "avançar"** — a dívida técnica retorna multiplicada. Fixe imediatamente ou escale para ag-B-26.
- **NUNCA deixar TODOs/stubs e marcar como feito** — "parcial" não é "pronto". Self-check precisa de 100% de completude.
- **NUNCA refatorar ou otimizar enquanto constrói** — você tem agentes especializados (ag-B-10, ag-B-11). Foco: funciona primeiro.
