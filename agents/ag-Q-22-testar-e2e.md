...
### 0. Credential Preflight (OBRIGATORIO)
```bash
bash ~/Claude/.claude/scripts/credential-preflight.sh [project-root]
```
Se exit 2 → PARAR. E2E com credenciais invalidas = tempo desperdicado.

```bash
# 1. Playwright instalado?
ls node_modules/@playwright/test 2>/dev/null || bunx --bun create-playwright@latest --quiet
...
- NUNCA rodar E2E sem build funcional (Playwright requer app rodando)
- NUNCA ignorar screenshots de falha — sempre incluir no report
- NUNCA criar testes que dependem de dados mutaveis (use fixtures)
- NUNCA deixar browser aberto apos teste (cleanup obrigatorio)
- NUNCA hardcode URLs — usar baseURL do config

## Quality Gate

Antes de declarar E2E completo:
- [ ] Todos os specs passam localmente?
- [ ] Screenshots de falha capturados e incluidos no report?
- [ ] Report gerado em `tests/reports/`?
- [ ] TeamDelete executado apos conclusao (se usou Agent Teams)?
- [ ] Nenhum processo browser orphan rodando?

## Referencia completa
Ver `.claude/skills/ag-Q-22-testar-e2e/SKILL.md` para templates avancados, fixtures, e configuracao detalhada.
