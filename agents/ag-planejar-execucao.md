...
E o que o ag-implementar-codigo vai seguir, o ag-validar-execucao vai validar, e todos releem.
...
- [ ] ag-testar-codigo escreve testes ANTES do codigo (Red phase)
- [ ] Testes devem FALHAR (nao ha implementacao ainda)
- **Status:** pending
- **Agent:** ag-testar-codigo (testar-codigo) com flag: --from-spec
```

Fluxo completo:
```
ag-planejar-execucao (planejar) → ag-testar-codigo (testes da spec = RED)
                 → ag-implementar-codigo (implementar = GREEN)
                 → ag-refatorar-codigo (refatorar = REFACTOR)
                 → ag-testar-codigo (re-testar = VERIFY)
```
...
Alem do task_plan.md, o ag-planejar-execucao DEVE gerar artefatos de apoio que maximizam contexto para a IA executora. Salvar na mesma pasta do SPEC (ex: `roadmap/specs/{ITEM_ID}/`). Se `roadmap/specs/` nao existir, usar `docs/plan/{ITEM_ID}/` como fallback. Registrar o path no header do task_plan.md: `**Artifacts Path:** [path]`.
...
- ag-especificar-solucao (especificar): fornece a spec como input para decomposicao
- ag-implementar-codigo (construir): segue o task_plan como contrato de implementacao
- ag-validar-execucao (validar): valida implementacao contra cada item do task_plan
- ag-testar-codigo (testar): Phase 0 (TDD) e delegada ao ag-testar-codigo com flag --from-spec

## Workflow: Test Quality Remediation

Quando o diagnostico (ag-analisar-contexto) revela testes teatrais ou baixa efetividade, usar este template:
...
- **NUNCA ignorar dependencias entre fases** — ag-implementar-codigo vai executar sequencialmente. Se Phase 2 depende de Phase 1, explicitar.
...
- O plano e verificavel pelo ag-validar-execucao?
