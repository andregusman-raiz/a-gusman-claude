...
E o que o ag-B-08 vai seguir, o ag-Q-12 vai validar, e todos releem.
...
- [ ] ag-Q-13 escreve testes ANTES do codigo (Red phase)
- [ ] Testes devem FALHAR (nao ha implementacao ainda)
- **Status:** pending
- **Agent:** ag-Q-13 (testar-codigo) com flag: --from-spec
```

Fluxo completo:
```
ag-P-07 (planejar) → ag-Q-13 (testes da spec = RED)
                 → ag-B-08 (implementar = GREEN)
                 → ag-B-10 (refatorar = REFACTOR)
                 → ag-Q-13 (re-testar = VERIFY)
```
...
Alem do task_plan.md, o ag-P-07 DEVE gerar artefatos de apoio que maximizam contexto para a IA executora. Salvar na mesma pasta do SPEC (ex: `roadmap/specs/{ITEM_ID}/`). Se `roadmap/specs/` nao existir, usar `docs/plan/{ITEM_ID}/` como fallback. Registrar o path no header do task_plan.md: `**Artifacts Path:** [path]`.
...
- ag-P-06 (especificar): fornece a spec como input para decomposicao
- ag-B-08 (construir): segue o task_plan como contrato de implementacao
- ag-Q-12 (validar): valida implementacao contra cada item do task_plan
- ag-Q-13 (testar): Phase 0 (TDD) e delegada ao ag-Q-13 com flag --from-spec

## Workflow: Test Quality Remediation

Quando o diagnostico (ag-P-04) revela testes teatrais ou baixa efetividade, usar este template:
...
- **NUNCA ignorar dependencias entre fases** — ag-B-08 vai executar sequencialmente. Se Phase 2 depende de Phase 1, explicitar.
...
- O plano e verificavel pelo ag-Q-12?
