...
- **raiz-data-engine**: 36 workflows (16 crons substituíveis por Railway cron, 5 CI substituíveis, 4 manuais migráveis)
- **raiz-platform**: 17 workflows (15 substituíveis por Vercel CI / Advanced Security / Supabase Functions)
- **8 projetos Next.js puros**: ci.yml redundante (Vercel CI já cobre)
- **6 essenciais identificados** no workspace inteiro: TOTVS MSSQL backfills, dre-raiz Fabric sync (ODBC), promo-scraper Python, raiz-platform CLI release, scraper VPS deploy, migration-governance + contracts
...
TOTVS MSSQL/ODBC/SOAP atrás de IP fixo whitelistado, drivers que Vercel/Railway runtime não suportam. Exemplo: `totvs-mssql-backfill.yml` (ADR-041 raiz-data-engine).

### W2 — Build/release de binário multi-plataforma
CLI distribuído via npm/Homebrew/winget exige matrix build (linux/macos/windows). Exemplo: `cli-release.yml` (raiz-platform).

### W3 — Deploy em VPS self-hosted
Sem integração Vercel/Railway. Exemplo: `deploy-scraper-service.yml` (raiz-platform).

### W4 — PR gate DB-first (psql + dbt + migrations)
Validação de schema/contracts que precisa de runner com psql + dbt instalado. Exemplo: `migration-governance.yml`, `contracts-validation.yaml` (raiz-data-engine). **Temporário** — migrar para Vercel CI quando viável.
...
|---|---|---|
| P0 | Remover ci.yml de Next.js puros com Vercel | 5 (juridico-raiz, auditoria-raiz, profdigital, fgts-platform, salarios-platform) |
| P0 | Migrar crons raiz-data-engine para Railway cron | 16 |
