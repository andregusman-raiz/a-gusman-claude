...
HARDCODED=$(grep -rl "example\|andregusman\|rAIz-AI-Prof\|rAIz_AI" \
...
for f in commands/ag*.md; do
  ...
  if echo "$content" | grep -q "ag-M-00-orquestrar\|ag-P-01-iniciar\|ag-P-02-setup\|ag-M-99-melhorar\|ag_skill-creator\|ag-Q-22-testar-e2e\|ag-Q-36-testar-manual\|ag-Q-37-gerar-testes\|ag-D-38-smoke-vercel"; then
    # Skill-based command - check skill dir exists
    skill_ref=$(echo "$content" | grep -oE 'ag-[A-Z]-[0-9]+-[a-z-]+|ag-M-99-[a-z-]+|ag_skill-creator|ui-ux-pro-max|nextjs-react|python-patterns|supabase-patterns|typescript-patterns' | head -1)
