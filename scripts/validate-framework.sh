...
HARDCODED_CRITICAL=$(grep -rl --exclude=validate-framework.sh "example\|andregusman\|rAIz-AI-Prof\|rAIz_AI" \
...
HARDCODED_CONTENT=$(grep -rl "example\|andregusman\|rAIz-AI-Prof\|rAIz_AI" \
  agents/ skills/ rules/ CLAUDE.md Playbooks/ 2>/dev/null || true)
if [[ -n "$HARDCODED_CONTENT" ]]; then
  count=$(echo "$HARDCODED_CONTENT" | wc -l | tr -d ' ')
  warn "$count markdown files contain business-domain refs (example/etc) — acceptable in content"
fi
