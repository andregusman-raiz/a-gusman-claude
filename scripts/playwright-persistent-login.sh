...
Projetos disponiveis:
  example-platform           http://localhost:3000      (Supabase + Google OAuth)
  example-prof             http://localhost:3001
  example-automata                http://localhost:3002
  edu-frontend       http://localhost:3003      (TOTVS Auth — sem SSO)
  edu-portal      http://localhost:3004
  fin-platform           http://localhost:3005
  agent-dashboard    http://localhost:4200
  attendance-app             http://localhost:3001      (TOTVS Auth — sem SSO)
  jusraiz                 https://app.jusraiz.com
  payroll-app       http://localhost:3000
  bi-app                 http://localhost:3000
...
Workflow recomendado para Google SSO:
  ...
  2. bash playwright-persistent-login.sh --sso example-platform
...
get_default_url() {
  case "$1" in
    example-platform)         echo "http://localhost:3000" ;;
    example-prof)           echo "http://localhost:3001" ;;
    example-automata)              echo "http://localhost:3002" ;;
    edu-frontend)     echo "http://localhost:3003" ;;
    edu-portal)    echo "http://localhost:3004" ;;
    fin-platform)         echo "http://localhost:3005" ;;
    agent-dashboard)  echo "http://localhost:4200" ;;
    attendance-app)           echo "http://localhost:3001" ;;
    jusraiz)               echo "https://app.jusraiz.com" ;;
    payroll-app)     echo "http://localhost:3000" ;;
    bi-app)               echo "http://localhost:3000" ;;
...
project_supports_sso() {
  case "$1" in
    edu-frontend|attendance-app|github|influency)
...
PLAYWRIGHT_PKG="$HOME/Claude/GitHub/example-platform/node_modules/playwright"
if [ ! -d "$PLAYWRIGHT_PKG" ]; then
  echo "Erro: playwright nao encontrado em $PLAYWRIGHT_PKG" >&2
  echo "Rode 'npm install' em ~/Claude/GitHub/example-platform/" >&2
  exit 3
fi
